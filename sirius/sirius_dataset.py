...
class Sirius(tfds.core.GeneratorBasedBuilder):
                ...
                'steps': tfds.features.Dataset({
                    'observation': tfds.features.FeaturesDict({
                        ...
                        'state': tfds.features.Tensor(
                            shape=(8,),
                            dtype=np.float32,
                            doc='Default robot state, consists of [7x robot joint state, '
                                '1x gripper state].',
                        ),
                        ...
                        'state_joint': tfds.features.Tensor(
                            shape=(7,),
                            dtype=np.float32,
                            doc='Robot 7-dof joint information.',
                        ),
                    ...
                    'action_mode': tfds.features.Tensor(
                        shape=(1,),
                        dtype=np.float32,
                        doc='Type of interaction. -1: initial human demonstration. '
                            '1: intervention. 0: autonomuos robot execution (includes pre-intervention class)',
                    ),
                    'intv_label': tfds.features.Tensor(
                        shape=(1,),
                        dtype=np.float32,
                        doc='Same as action_modes, except 15 timesteps preceeding intervention are labeled as -10.'
                    ),
    ...
    def _generate_examples(self) -> Iterator[Tuple[str, Any]]:
        ...
        def _parse_example(f, demo_key, language_instruction, ds_name, ds_path, ac_type):
            ...
            for i in range(ep_len):
                ...
                episode.append({
                    'observation': {
                        'image': np.flip(data["obs"]['agentview_image'][i], axis=2),
                        'wrist_image': np.flip(data["obs"]['eye_in_hand_image'][i], axis=2),
                    ...
                    'action_mode': np.array([data['action_modes'][i]], dtype=np.float32),
                    'intv_label': np.array([data['intv_labels'][i]], dtype=np.float32),
                    'discount': 1.0,
        ...
        dataset_specs = dict(
            gear=dict(
                path="/home/soroush/datasets/sirius_data/gear/round0123.hdf5",
                language_instruction="Insert the blue gear onto the right peg, followed by the red gear",
                ac_type="pos_gripper",
            ),
            kcup=dict(
                path="/home/soroush/datasets/sirius_data/kcup/round0123.hdf5",
                language_instruction="Open the kcup holder, insert the kcup into the holder, and close the kcup holder",
                ac_type="pos_yaw_gripper",
            ),
        )
